...
const ChartComponent = () => {
  ...
  const defaultAssets = [...new Set(responseData.map(item => item.Item))].slice(0, 3);
  ...
  useEffect(() => {
    const validYears = responseData.map(item => item.Year).filter(year => year !== undefined);
  ...
  const updateChartData = (selectedAssets, selectedYear) => {
    ...
    }).filter(item => selectedAssets.some(asset => item[asset] > 0));
  
    setChartData(mergedData);
  };
 
const updateChartData2 = (selectedAssets, selectedYear) => {
  let filteredUpdatedData2 = responseData;
  ...
  // Filter out months with all zero values
  const filteredMergedData2 = mergedData2.filter(item => {
    return selectedAssets.some(asset => item[`${asset}_updated2`] !== 0 || item[`${asset}_previous2`] !== 0);
  });
 
  setChartData2(filteredMergedData2);
};
  ...
  const assetOptions = [...new Set(responseData.map(item => item.Item))].map(asset => ({
    value: asset,
    label: asset,
  }));
 
  const yearOptions = [...new Set(responseData.map(item => item.Year))]
  ...
  useLayoutEffect(() => {
    ...
    selectedAssets.forEach((asset, index) => {
      ...
      label.locationY = -0.03; // Position the label at the top of the bar
  ...
  useLayoutEffect(() => {
    ...
    chart2.legend.labels.template.fontSize = 18; // Adjusting font size based on number of assets
